-- Preserve Eddy's raw values while correcting duplicated contract-level
-- negative balances for payee-level Eddy Master Run calculations.

ALTER TABLE eddy_master_statements
  ADD COLUMN IF NOT EXISTS net_payee_subtotal_auto_corrected NUMERIC(24, 12),
  ADD COLUMN IF NOT EXISTS net_payee_subtotal_correction_reason TEXT;
...
ALTER TABLE eddy_master_statements
  ADD CONSTRAINT eddy_master_statements_auto_correction_valid CHECK (
    ...
    OR (
      net_payee_subtotal < 0
      AND eddy_payee_split_percent IS NOT NULL
      AND net_payee_subtotal_auto_corrected = ROUND(net_payee_subtotal * eddy_payee_split_percent / 100, 12)
    )
  );
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE((v_row->>'duplicate_negative_correction')::BOOLEAN, FALSE) THEN
      IF v_raw_net >= 0 OR v_split_percent IS NULL THEN
        RAISE EXCEPTION 'Invalid duplicated-negative correction for Eddy Statement ID %', v_row->>'statement_id';
      END IF;
      v_auto_corrected_net := ROUND(v_raw_net * v_split_percent / 100, 12);
      v_correction_reason := format(
        'Adjusted from duplicated Eddy contract balance using %s%% payee split',
        v_split_percent
      );
    END IF;
    ...
    IF v_existing_statement_id IS NULL THEN
      INSERT INTO eddy_master_statements (
        ...
        v_correction_reason, v_split_percent
      );
    ELSE
...
COMMENT ON COLUMN eddy_master_statements.net_payee_subtotal_auto_corrected IS
  'Payee-specific value calculated only when Eddy repeats one negative contract balance across multiple payees.';
COMMENT ON COLUMN eddy_master_statements.net_payee_subtotal_correction_reason IS
  'Audit explanation for the automatic duplicated-negative correction.';
